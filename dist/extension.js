...
const dependencyTracker_1 = __webpack_require__(2);
const pyProjectParser_1 = __webpack_require__(6);
const uvLockParser_1 = __webpack_require__(22);
const versionChecker_1 = __webpack_require__(23);
const codeLensProvider_1 = __webpack_require__(38);
const hoverProvider_1 = __webpack_require__(39);
let dependencyTracker;
function activate(context) {
    console.log("Python Dependency Tracker activated");
    const pyProjectParser = new pyProjectParser_1.PyProjectParser();
    const uvLockParser = new uvLockParser_1.UvLockParser();
    const versionChecker = new versionChecker_1.VersionChecker();
    const codeLensProvider = new codeLensProvider_1.DependencyCodeLensProvider();
    const hoverProvider = new hoverProvider_1.DependencyHoverProvider();
    dependencyTracker = new dependencyTracker_1.DependencyTracker(pyProjectParser, uvLockParser, versionChecker, codeLensProvider, hoverProvider);
    // Register CodeLens provider
    const codeLensDisposable = vscode.languages.registerCodeLensProvider({ language: "toml", pattern: "**/pyproject.toml" }, codeLensProvider);
    // Register Hover provider
    const hoverDisposable = vscode.languages.registerHoverProvider({ language: "toml", pattern: "**/pyproject.toml" }, hoverProvider);
    // Register commands
    const refreshCommand = vscode.commands.registerCommand("pythonDependencyTracker.refreshVersions", () => dependencyTracker.refreshVersions());
    const toggleShowVersionsCommand = vscode.commands.registerCommand("pythonDependencyTracker.toggleShowVersions", () => dependencyTracker.toggleShowVersions());
    const bumpToInstalledCommand = vscode.commands.registerCommand("pythonDependencyTracker.bumpToInstalled", (dependency) => dependencyTracker.bumpToInstalled(dependency));
    const upgradeToLatestCommand = vscode.commands.registerCommand("pythonDependencyTracker.upgradeToLatest", (dependency) => dependencyTracker.upgradeToLatest([dependency]));
    const bumpAllCommand = vscode.commands.registerCommand("pythonDependencyTracker.bumpAll", () => dependencyTracker.bumpAll());
    const upgradeAllCommand = vscode.commands.registerCommand("pythonDependencyTracker.upgradeAll", () => dependencyTracker.upgradeAll());
    const openPackagePageCommand = vscode.commands.registerCommand("pythonDependencyTracker.openPackagePage", (dependency) => {
        const url = dependency.homePage || dependency.packageUrl;
        if (url) {
            vscode.env.openExternal(vscode.Uri.parse(url));
        }
    });
    context.subscriptions.push(codeLensDisposable, hoverDisposable, refreshCommand, toggleShowVersionsCommand, bumpToInstalledCommand, upgradeToLatestCommand, bumpAllCommand, upgradeAllCommand, openPackagePageCommand);
    ...
    const fileWatcher = vscode.workspace.createFileSystemWatcher(new vscode.RelativePattern(vscode.workspace.workspaceFolders?.[0] || "", "{pyproject.toml,uv.lock}"));
...
const projectRoot_1 = __webpack_require__(5);
class DependencyTracker {
    constructor(pyProjectParser, uvLockParser, versionChecker, codeLensProvider, hoverProvider) {
        this.pyProjectParser = pyProjectParser;
    ...
    async refreshVersions() {
        ...
        const projectRoot = await (0, projectRoot_1.resolveProjectRoot)();
        if (!projectRoot) {
        ...
        console.log("DependencyTracker: Found dependencies in pyproject.toml:", this.dependencies.length);
        // Parse uv.lock
        console.log("DependencyTracker: Parsing uv.lock");
        const installedPackages = this.uvLockParser.parseInstalledPackages(projectRoot);
        console.log("DependencyTracker: Found installed packages in uv.lock:", installedPackages.size);
        ...
        for (const dep of this.dependencies) {
            ...
            try {
                const versionInfo = await this.versionChecker.getLatestVersion(dep.name, dep.installedVersion || dep.requestedVersion);
                if (versionInfo) {
        ...
        console.log("DependencyTracker: Processed dependencies:", this.dependencies.length);
    ...
    async checkOutdated() {
        ...
        const items = outdated.map((dep) => ({
            label: dep.name,
            description: `${dep.installedVersion || dep.requestedVersion} → ${dep.latestVersion}`,
        ...
        if (selected) {
            vscode.env.openExternal(vscode.Uri.parse(`https://pypi.org/project/${selected.label}/`));
        }
    }
    setupEditorListeners() {
        ...
        const onDidChangeActiveEditor = vscode.window.onDidChangeActiveTextEditor((editor) => {
            if (editor) {
                console.log("DependencyTracker: Active editor changed in setupEditorListeners:", editor.document.fileName);
                // CodeLens is updated automatically when active editor changes
            }
        });
        const onDidChangeTextDocument = vscode.workspace.onDidChangeTextDocument((event) => {
            console.log("DependencyTracker: Document changed:", event.document.fileName);
            if (event.document.fileName.endsWith("pyproject.toml")) {
                console.log("DependencyTracker: pyproject.toml changed, updating in 1 second");
    ...
    setupAutoRefresh() {
        ...
        const enableAutoCheck = config.get("enableAutoCheck", true);
        const checkInterval = config.get("checkInterval", 300000);
        if (enableAutoCheck) {
    ...
    updateCodeLens() {
        this.codeLensProvider.updateDependencies(this.dependencies, this.pyprojectPath);
        this.hoverProvider.updateDependencies(this.dependencies, this.pyprojectPath);
    }
    async toggleShowVersions() {
        await this.codeLensProvider.toggleShowVersions();
    }
    async updateToInstalled(dependency) {
        if (!dependency || !dependency.installedVersion) {
            return;
        }
        await this.updateVersionInFile(dependency);
    }
    async updateToLatest(dependency) {
        if (!dependency || !dependency.latestVersion) {
            return;
        }
        await this.updateVersionInFile(dependency);
    }
    async bumpToInstalled(dependency) {
        if (!dependency || !dependency.installedVersion) {
            return;
        }
        await this.updateVersionInFile(dependency);
    }
    async upgradeToLatest(dependency) {
        if (!this.projectRoot) {
            return;
        }
    ...
    async bumpAll() {
        const needsBump = this.dependencies.filter((dep) => dep.installedVersion && dep.requestedVersion !== dep.installedVersion);
        if (needsBump.length === 0) {
            vscode.window.showInformationMessage("Нет пакетов для обновления до установленной версии");
            return;
        }
        for (const dep of needsBump) {
            await this.updateVersionInFile(dep);
        }
        vscode.window.showInformationMessage(`Обновлено ${needsBump.length} пакетов до установленной версии`);
    }
    async upgradeAll() {
        const needsUpgrade = this.dependencies.filter((dep) => dep.latestVersion && dep.installedVersion !== dep.latestVersion);
        if (needsUpgrade.length === 0) {
            vscode.window.showInformationMessage("Нет пакетов для обновления до последней версии");
            return;
        }
    ...
    async updateVersionInFile(dependency) {
        ...
        try {
            ...
            vscode.window.showInformationMessage(`Updated ${dependency.name} to version ${dependency.installedVersion}`);
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.resolveProjectRoot = exports.getProjectRootFromDocument = void 0;
const fs = __importStar(__webpack_require__(3));
const path = __importStar(__webpack_require__(4));
const vscode = __importStar(__webpack_require__(1));
function isProjectManifestDocument(document) {
    const baseName = path.basename(document.fileName);
    return baseName === "pyproject.toml" || baseName === "uv.lock";
}
function getProjectRootFromDocument(document) {
...
exports.getProjectRootFromDocument = getProjectRootFromDocument;
function getProjectRootFromEditors() {
...
function getProjectRootFromWorkspaceFolders() {
    const workspaceFolders = vscode.workspace.workspaceFolders;
    if (!workspaceFolders) {
        return undefined;
    }
...
async function findProjectRootInWorkspace() {
    const pyprojectFiles = await vscode.workspace.findFiles("**/pyproject.toml", "{**/node_modules/**,**/.venv/**,**/.git/**}", 1);
...
const fs = __importStar(__webpack_require__(3));
const path = __importStar(__webpack_require__(4));
const toml = __importStar(__webpack_require__(7));
class PyProjectParser {
    ...
    parseProjectDependencies(parsed, lines, dependencies) {
    ...
    parseDependencyGroups(parsed, lines, dependencies) {
    ...
    parseDependencyList(depList, source, lines, dependencies) {
    ...
    parseDependencyString(depString, source, index, lines) {
    ...
    findDependencyLine(lines, depString) {
...
module.exports.stream = __webpack_require__(19)
module.exports.prettyError = __webpack_require__(17)
...
const TOMLParser = __webpack_require__(10)
const prettyError = __webpack_require__(17)
...
module.exports = makeParserClass(__webpack_require__(11))
...
const createDateTime = __webpack_require__(12)
const createDateTimeFloat = __webpack_require__(13)
const createDate = __webpack_require__(15)
...
const f = __webpack_require__(14)
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.UvLockParser = void 0;
const fs = __importStar(__webpack_require__(3));
const path = __importStar(__webpack_require__(4));
const toml = __importStar(__webpack_require__(7));
class UvLockParser {
    parseInstalledPackages(projectRoot) {
...
exports.UvLockParser = UvLockParser;
...
const node_fetch_1 = __importDefault(__webpack_require__(24));
class VersionChecker {
    constructor() {
        this.cache = new Map();
        this.CACHE_TTL = 5 * 60 * 1000; // 5 minutes
    }
    async getLatestVersion(packageName, currentVersion) {
        // Check cache
        ...
        try {
            const response = await (0, node_fetch_1.default)(`https://pypi.org/pypi/${packageName}/json`);
            if (!response.ok) {
                return null;
            }
...
/* harmony import */ var https__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(36);
/* harmony import */ var zlib__WEBPACK_IMPORTED_MODULE_5__ = __webpack_require__(37);
...
exports.URL = __webpack_require__(28)["interface"];
...
const Impl = __webpack_require__(31);
...
const punycode = __webpack_require__(33);
const tr46 = __webpack_require__(34);
...
var mappingTable = __webpack_require__(35);
...
class DependencyCodeLensProvider {
    constructor() {
        ...
        const config = vscode.workspace.getConfiguration("pythonDependencyTracker");
        this.showVersions = config.get("showVersions", true);
    }
    updateDependencies(dependencies, pyprojectPath) {
        this.dependencies = dependencies;
        this.pyprojectPath = pyprojectPath;
        console.log("CodeLensProvider: Updating dependencies:", dependencies.length);
        this._onDidChangeCodeLenses.fire();
    }
    async saveShowVersionsState() {
        const config = vscode.workspace.getConfiguration("pythonDependencyTracker");
        await config.update("showVersions", this.showVersions, vscode.ConfigurationTarget.Workspace);
    }
    async toggleShowVersions() {
    ...
    provideCodeLenses(document, token) {
        ...
        console.log("CodeLensProvider: Number of dependencies:", this.dependencies.length);
        ...
        if (this.showVersions) {
            for (const dep of this.dependencies) {
                if (dep.line >= 0 && dep.line < document.lineCount) {
                    ...
                    console.warn(`CodeLensProvider: Incorrect line number ${dep.line} for dependency ${dep.name}, total lines: ${document.lineCount}`);
                }
            }
        }
    ...
    createMassOperationsCodeLenses() {
        const codeLenses = [];
        const range = new vscode.Range(0, 0, 0, 0);
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.DependencyHoverProvider = void 0;
const vscode = __importStar(__webpack_require__(1));
const path = __importStar(__webpack_require__(4));
class DependencyHoverProvider {
    ...
    findDependencyInLine(lineText, character) {
...
exports.DependencyHoverProvider = DependencyHoverProvider;
